...
let HandlerService = class HandlerService {
    ...
    async importHandler(handler, tenantId) {
        const handlerPath = tenantId
            ? path.resolve(this.configService.get('paths').manifestFolder, tenantId, 'handlers', `${handler}.js`)
            : path.resolve(this.configService.get('paths').handlersFolder, `${handler}.js`);
